...
- [ ] **11. Review hosting configuration.** Verify `amplify.yml` builds the nested root, artifacts are `dist/`, and no secret values are defined in source-controlled configuration. Verify `.env.local` remains ignored.
